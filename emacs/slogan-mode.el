;;; slogan.el --- Major mode for editing Slogan source text
;;; Copied and updated a javascript-mode :-)
;;; Need help to improve this or to write a new mode from scratch.
...
;;; Commentary:
;;
;; The main features of this Slogan mode are syntactic
...
;; 
;;    (add-to-list 'auto-mode-alist '("\\.sn\\'" . slogan-mode))
;;    (autoload 'slogan-mode "slogan" nil t)
;;    
;; General Remarks:
;; 
...
;; 
;; Exported names start with "slogan-" whereas private names start
;; with "js-".
;; 
;; Changes:
;;
;; See slogan.el.changelog.
...
(defgroup slogan nil 
  "Customization variables for `slogan-mode'."
  :tag "Slogan"
  :group 'languages)

(defcustom slogan-indent-level 4
  "Number of spaces for each indentation step."
  :type 'integer
  :group 'slogan)

(defcustom slogan-expr-indent-offset 0
...
(defcustom slogan-auto-indent-flag t
...
(defvar slogan-mode-map nil 
  "Keymap used in Slogan mode.")

(unless slogan-mode-map 
  (setq slogan-mode-map (make-sparse-keymap)))

(when slogan-auto-indent-flag
  (mapc (lambda (key) 
	  (define-key slogan-mode-map key 'slogan-insert-and-indent))
	'("{" "}" "(" ")" ":" ";" ",")))

(defun slogan-insert-and-indent (key)
...
(defvar slogan-mode-syntax-table
  (let ((table (make-syntax-table)))
    ...
    table)
  "Syntax table used in Slogan mode.")

(defvar js-ident-as-word-syntax-table
  (let ((table (copy-syntax-table slogan-mode-syntax-table)))
...
(defconst js-method-heading-2-re 
  "^[ \t]*\\(\\w+\\)[ \t]*:[ \t]*method\\>"
  "Regular expression matching the start of a function entry in
  an associative array.")

(defconst js-keyword-re
  (regexp-opt '("fn" "function" "method" "define" "record"
		"if" "else" "let" "letseq" "letrec" 
		"case" "match" "where" "try" "trycc" "catch" "finally"
		"macro" "namespace" "import" "declare") 'words)
  "Regular expression matching any Slogan keyword.")

(defconst js-basic-type-re
  (regexp-opt '() 'words)
  "Regular expression matching any predefined type in Slogan.")

(defconst js-constant-re
  (regexp-opt '("false" "nil" "true" "cases") 'words)
  "Regular expression matching any future reserved words in Slogan.")
...
(defconst js-font-lock-keywords-1
  (list 
   ...
   (list js-method-heading-2-re 1 font-lock-function-name-face)
  ...
  (save-excursion
    ...
    (when (save-excursion
      ...
      (save-excursion
        ...
        (+ (current-indentation) slogan-indent-level)))))
             ...
             (if (looking-at "[({[][ \t]*\\(/[/*]\\|$\\)")
                 (progn
                   ...
                   (cond (same-indent-p
                          (current-column))
                         (continued-expr-p
                          (+ (current-column) (* 2 slogan-indent-level)
                             slogan-expr-indent-offset))
                         (t
                          (+ (current-column) slogan-indent-level))))
               (unless same-indent-p
                 (forward-char)
                 (skip-chars-forward " \t"))
               (current-column)))
	    (continued-expr-p (+ slogan-indent-level 
                                 slogan-expr-indent-offset))
            (t 0)))))


(defun slogan-indent-line ()
  "Indent the current line as Slogan source text."
  (interactive)
...
(defun slogan-fill-paragraph (&optional justify)
...
;;;###autoload
(defun slogan-mode ()
  "Major mode for editing Slogan source text.
...
\\{slogan-mode-map}"
  ...
  (use-local-map slogan-mode-map)
  (set-syntax-table slogan-mode-syntax-table)
  (set (make-local-variable 'indent-line-function) 'slogan-indent-line)
  ...
  (setq major-mode 'slogan-mode)
  (setq mode-name "Slogan")
  (run-hooks 'slogan-mode-hook))
