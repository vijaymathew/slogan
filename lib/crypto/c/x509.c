...
void init_x509_certificate(signed_x509_certificate *certificate)
{
  ...
  certificate->tbsCertificate.subjectPublicKeyInfo.rsa_public_key.modulus = malloc(sizeof(huge));
  certificate->tbsCertificate.subjectPublicKeyInfo.rsa_public_key.exponent =  malloc(sizeof(huge));
  init_huge(certificate->tbsCertificate.subjectPublicKeyInfo.rsa_public_key.modulus);
  init_huge(certificate->tbsCertificate.subjectPublicKeyInfo.rsa_public_key.exponent);
  set_huge(certificate->tbsCertificate.subjectPublicKeyInfo.rsa_public_key.modulus, 0);
  set_huge(certificate->tbsCertificate.subjectPublicKeyInfo.rsa_public_key.exponent, 0);
...
{
  release_huge(&certificate->tbsCertificate.serialNumber);
  free_x500_name(&certificate->tbsCertificate.issuer);
  free_x500_name(&certificate->tbsCertificate.subject);
  release_huge(certificate->tbsCertificate.subjectPublicKeyInfo.rsa_public_key.modulus);
  release_huge(certificate->tbsCertificate.subjectPublicKeyInfo.rsa_public_key.exponent);
  free(certificate->tbsCertificate.subjectPublicKeyInfo.rsa_public_key.modulus);
  free(certificate->tbsCertificate.subjectPublicKeyInfo.rsa_public_key.exponent);
  release_huge(&certificate->tbsCertificate.subjectPublicKeyInfo.dsa_parameters.g);
  release_huge(&certificate->tbsCertificate.subjectPublicKeyInfo.dsa_parameters.p);
  release_huge(&certificate->tbsCertificate.subjectPublicKeyInfo.dsa_parameters.q);
  release_huge(&certificate->tbsCertificate.subjectPublicKeyInfo.dsa_public_key);
  release_huge(&certificate->rsa_signature_value);
  release_huge(&certificate->dsa_signature_value.r);
  release_huge(&certificate->dsa_signature_value.s);
  release_huge(&certificate->tbsCertificate.subjectUniqueId);
  release_huge(&certificate->tbsCertificate.issuerUniqueId);
  if (certificate->hash != NULL) free(certificate->hash);
}
  ...
  while (typeValuePair)
    {
      ...
      if (!memcmp(type->data, OID_idAtCountryName, type->length))
      ...
      else if (!memcmp(type->data, OID_idAtOrganizationalUnitName, type->length)) 
        {
          target->idAtOrganizationalUnitName = (char *) malloc(value->length + 1);
      ...
      else
        {
          ...
          for (i = 0; i < type->length; i++)
            {
              fprintf(stderr, "%.02x ", type->data[i]);
            }
          fprintf(stderr, "\n");
        }
...
static int parse_extension(x509_certificate *certificate, struct asn1struct *source)
{
  struct asn1struct *oid;
...
static int parse_extensions(x509_certificate *certificate, struct asn1struct *source)
{
...
static int parse_tbs_certificate(x509_certificate *target, struct asn1struct *source)
{
  ...
  if (parse_algorithm_identifier(&target->signature, signatureAlgorithmIdentifier)) return 3;
  ...
  if (parse_public_key_info(&target->subjectPublicKeyInfo, publicKeyInfo)) return 7;
  if (extensions)
    {
...
{
  return dsa_verify(&certificate->tbsCertificate.subjectPublicKeyInfo.dsa_parameters,
 ...
 */
static int parse_rsa_signature_value(signed_x509_certificate *target, struct asn1struct *source)
{
...
static int parse_dsa_signature_value(signed_x509_certificate *target, struct asn1struct *source)
{
                           ...
                           signed_x509_certificate *parsed_certificate)
{
  ...
  parsed_certificate->hash = (unsigned int *)malloc(sizeof(unsigned int) * digest.hash_len);
  assert(parsed_certificate->hash != NULL);
  memcpy(parsed_certificate->hash, digest.hash, sizeof(unsigned int) * digest.hash_len);
  parsed_certificate->hash_len = digest.hash_len;
  
  switch (parsed_certificate->algorithm)
    {
    case md5WithRSAEncryption:
      release_md5_digest(&digest);
      break;
    case shaWithRSAEncryption:
    case shaWithDSA:
      release_sha1_digest(&digest);
      break;
    default:
      break;
    }
...
static void init_name(name *n)
{
  n->idAtCountryName = NULL;
  n->idAtStateOrProvinceName = NULL;
  n->idAtLocalityName = NULL;
  n->idAtOrganizationName = NULL;
  n->idAtOrganizationalUnitName = NULL;
  n->idAtCommonName = NULL;
}

static void init_pkey_info(public_key_info *p)
{
  p->algorithm = rsa;
  p->rsa_public_key.modulus = NULL;
  p->rsa_public_key.exponent = NULL;
  init_huge(&p->dsa_parameters.g);
  init_huge(&p->dsa_parameters.p);
  init_huge(&p->dsa_parameters.q);
  init_huge(&p->dsa_public_key);
}

static void init_signed_x509_certificate(signed_x509_certificate *cert)
{
  cert->hash = NULL;
  cert->hash_len = 0;
  cert->algorithm = md5WithRSAEncryption;
  init_huge(&cert->rsa_signature_value);
  init_huge(&cert->dsa_signature_value.r);
  init_huge(&cert->dsa_signature_value.s);
  
  cert->tbsCertificate.version = 0;
  init_huge(&cert->tbsCertificate.serialNumber);
  cert->tbsCertificate.signature = md5WithRSAEncryption;
  init_name(&cert->tbsCertificate.issuer);
  cert->tbsCertificate.validity.notBefore = 0;
  cert->tbsCertificate.validity.notAfter = 0;
  init_name(&cert->tbsCertificate.subject);
  init_pkey_info(&cert->tbsCertificate.subjectPublicKeyInfo);
  init_huge(&cert->tbsCertificate.issuerUniqueId);
  init_huge(&cert->tbsCertificate.subjectUniqueId);
  cert->tbsCertificate.certificate_authority = 0;
}
                        ...
                        ___slogan_obj iis_pem)
{
  ...
  init_signed_x509_certificate(certificate);
