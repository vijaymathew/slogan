  ...
  if (quotient)
    {
      ...
      if (quotient->rep != NULL) free(quotient->rep);
...
void inv(huge *z, huge *a)
{
  ...
  release_huge(&y1);
  release_huge(&y);
  release_huge(&quotient);
  release_huge(&remainder);
  release_huge(&a_temp);
}
