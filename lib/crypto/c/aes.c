...
void crypto_aes(___slogan_obj str_key,
                ...
                ___slogan_obj i_encrypt)
{
  ...
  if (strlen(key) == 32) aes256 = 1;

  if (encrypt)
    if (aes256 == 1)
      aes_256_encrypt((unsigned char *)___BODY(u8arr_input),
                      input_len,
                      (unsigned char *)___BODY(u8arr_output),
                      iv, key);
    else // keylen is 16 bytes
      aes_128_encrypt((unsigned char *)___BODY(u8arr_input),
                      input_len,
                      (unsigned char *)___BODY(u8arr_output),
                      iv, key);
  else
    if (aes256 == 1)
      aes_256_decrypt((unsigned char *)___BODY(u8arr_input),
                      input_len,
                      (unsigned char *)___BODY(u8arr_output),
                      iv, key);
    else // keylen is 16 bytes
      aes_128_decrypt((unsigned char *)___BODY(u8arr_input),
                      input_len,
                      (unsigned char *)___BODY(u8arr_output),
                      iv, key);
}
