...
(define (add-def-to-namespace expr)
  (if (and (pair? expr) (not (null? expr)))
      (if (and (eq? (car expr) 'define)
          ...
          (let ((snamespace-name (symbol->string (caar *namespaces*)))
            ...
            (append `(define ,(string->symbol (string-append snamespace-name "_" svar)))
                    (intern-to-top-namespace (cddr expr))))
          (intern-to-top-namespace expr))
      (intern-to-top-namespace expr)))

(define (intern-to-top-namespace expr)
