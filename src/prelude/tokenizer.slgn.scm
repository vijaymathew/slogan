...
(define (read-name port)
  (cond ((char-valid-name-start? (port-pos-peek-char port))
        ...
        ((char=? #\` (port-pos-peek-char port))
         (port-pos-read-char! port)
         (let ((opr (single-char-operator? (port-pos-peek-char port))))
           (let ((fn (cond (opr
                            (port-pos-read-char! port)
                            (quoted-opr->fname (cdr opr)))
                           ((multi-char-operator? (port-pos-peek-char port))
                            (quoted-opr->fname (read-multi-char-operator port)))
                           (else (tokenizer-error "Misplaced backquote.")))))
             (if (not (char=? #\` (port-pos-read-char! port)))
                 (tokenizer-error "Missing closing backquote.")
                 fn))))
        (else
         (tokenizer-error "read-name failed at " (port-pos-read-char! port)))))

(define (quoted-opr->fname opr)
  (case opr
    ((*plus*) 'add)
    ((*minus*) 'sub)
    ((*asterisk*) 'mult)
    ((*backslash*) 'div)
    ((*equals*) 'equal?)
    ((*less-than*) 'number_is_lt)
    ((*less-than-equals*) 'number_is_lteq)
    ((*greater-than*) 'number_is_gt)
    ((*greater-than-equals*) 'number_is_gteq)
    ((*and*) 'and)
    ((*or*) 'or)
    (else (tokenizer-error "Operator cannot be quoted."))))
