...
(define (read-special-number port opr)
  (let ((sym (cdr opr)))
    (if (or (eq? sym '*plus*)
	    (eq? sym '*minus*))
	(let ((c (peek-char port)))
	  (if (special-number-start? c)
	      (read-special-number-helper port (car opr))
	      sym))
	sym)))
  
(define (special-number-start? c)
  (or (char=? c #\i)
      (char=? c #\n)))

(define (char-valid-in-special-number? c)
  (or (char=? c #\i)
      (char=? c #\n)
      (char=? c #\f)
      (char=? c #\n)
...
(define (read-special-number-helper port prefix)
  (let loop ((c (peek-char port))
             (result (if prefix (list prefix) '())))
    (if (char-valid-in-special-number? c)
        (begin (read-char port)
               (loop (peek-char port)
                     (cons c result)))
	(let ((n (string->number (list->string (reverse result)))))
