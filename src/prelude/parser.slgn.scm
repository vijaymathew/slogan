...
(define (expression/statement tokenizer)
  (if (eof-object? (tokenizer 'peek))
      (tokenizer 'next)
      (let ((v (add-def-to-namespace (statement tokenizer))))
        (if (not v) (set! v (intern-to-top-namespace (expression tokenizer))))
      ...
      (begin (tokenizer 'next)
             (let ((name (tokenizer 'next)))
               (if (eq? name 'lazy)
                   (let ((name (tokenizer 'next)))
                     (if (not (name? name))
                         (parser-error tokenizer "Expected name to declare lazy.")
                         (declare-lazy name)))
...
(define (import-defs tokenizer)
  (let ((token (tokenizer 'peek)))
    (cond ((eq? token '*open-paren*)
           (tokenizer 'next)
           (import-defs-list tokenizer))
          ((name? token)
           (tokenizer 'next))
          (else (parser-error tokenizer "Invalid import statement.")))))
...
(define (import-defs-list tokenizer)
  (let loop ((token (tokenizer 'next))
    ...
    (cond ((eq? token '*close-paren*)
          ...
          (else (parser-error tokenizer "Invalid import list entry.")))))
		 ...
		 (cond ((eq? token '?)
			...
			(list 'rvar))
		       (else (let ((var (tokenizer 'next)))
                ...
                ((eq? token '*bang*)
                 (tokenizer 'next)
		 (let ((expr (tokenizer 'next)))
		   (if (not (symbol? expr))
		       (parser-error tokenizer "Expected symbol."))
		   `(quote ,expr)))
		((eq? token '*quote-start*)
		 (tokenizer 'next)
