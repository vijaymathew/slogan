        ...
        (let ((paramtypes (expression tokenizer)))
          ...
          (if (not (eq? (car paramtypes) 'list))
              ...
              (let ((name fn-name))
                ...
                (def-c-fn libname fn-name name (cdr paramtypes) rettype)))))))
...
(define (safe-cmpr-expr? expr)
  (and (pair? expr)
       (memq (car expr) '(safe-equal? safe-< safe-<= safe-> safe->=))))
...
(define (array-literal tokenizer)
  (let ((prefix (tokenizer 'peek)))
    ...
    (if (eq? (tokenizer 'peek) '*open-bracket*)
        (begin (tokenizer 'next)
               (let loop ((expr (list (array-prefix->constructor prefix)))
...
(define (table-literal tokenizer)
  ...
  (let loop ((expr '(let ((*table* (make_equal_hashtable))))))
    (if (eq? (tokenizer 'peek) '*close-brace*)
        (begin (tokenizer 'next)
               (append expr '(*table*)))
        (let ((keyval (expression tokenizer)))
          (if (not (and (pair? keyval)
              ...
              (begin (assert-comma-separator tokenizer '*close-brace*)
                     (loop (append expr (list `(hashtable_set *table* ,(cadr keyval) ,(caddr keyval)))))))))))
