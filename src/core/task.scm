...
(define (safe-condition-variable-broadcast! cv mtx)
...
(define (rbind dfv value)
  (let ((mtx (reactive-var-mtx dfv)))
    ...
    (let* ((cv (reactive-var-cv dfv))
           (datum (condition-variable-specific cv))
           (err #f))
      (if (and (scm-not (unbound? datum))
               (scm-not (scm-is_equal value datum)))
          (begin (mutex-unlock! mtx)
                 (scm-error "cannot rebind reactive variable to a new value"))
          (begin (condition-variable-specific-set! cv value)
                 (safe-condition-variable-broadcast! cv mtx))))))
