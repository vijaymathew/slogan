...
(define (%bitvector-vect-size bitvect-size)
  (scm-+ (if (scm-> (scm-remainder bitvect-size %bitvect-bpi%) 0) 1 0)
...
(define (slgn-bit_array . rest)
  (let loop ((rest rest)
    ...
    (if (null? rest) ba
        (begin (if (scm-not (scm-= 0 (scm-car rest)))
                   (bitvector-set! ba i))
               (loop (scm-cdr rest) (scm-+ i 1) ba)))))
...
(define (bitvector-set! self i)
  (let ((vec (%bitvector-vector self))
    ...
    (u32vector-set! vec idx (bitwise-ior (u32vector-ref %bitvect-masks% (scm-- i (scm-* %bitvect-bpi% idx)))
...
(define (bitvector-set? self i)
  (let ((vec (%bitvector-vector self))
        (idx (fxwraplogical-shift-right i %bitvect-shift%)))
    (scm-not (zero? (bitwise-and (u32vector-ref %bitvect-masks% (scm-- i (scm-* %bitvect-bpi% idx)))
                             (u32vector-ref vec idx))))))
...
(define (bitvector-clear! self i)
  (let ((vec (%bitvector-vector self))
        (idx (fxwraplogical-shift-right i %bitvect-shift%)))
    (u32vector-set! vec idx (bitwise-and (bitwise-not (u32vector-ref %bitvect-masks% (scm-- i (scm-* %bitvect-bpi% idx))))
                                         (u32vector-ref vec idx)))))
...
(define (%bitvector-set-all! self v)
  (let ((vect (%bitvector-vector self)))
    (let loop ((i (scm-- (%bitvector-vector-length self) 1)))
      (cond ((scm->= i 0)
...
(define (bitvector-all-set? self)
  (let ((len (%bitvector-vector-length self))
    ...
    (let loop ((res #t)
               (i 0))
      (if (and res (scm-< i len))
          (loop (scm-= %bitvect-all-on% (u32vector-ref v i)) (scm-+ i 1))
          res))))
...
(define (bitvector-one-set? self)
  (let ((len (%bitvector-vector-length self))
    ...
    (let loop ((res #t)
               (i 0))
      (if (and res (scm-< i len))
          (loop (scm-= %bitvect-all-off% (u32vector-ref v i)) (scm-+ i 1))
          (scm-not res)))))
  ...
  (let ((slen (%bitvector-vector-length self))
    ...
    (if (scm-= slen tlen)
        (let ((svec (%bitvector-vector self))
          ...
          (let loop ((i 0)
                     (res #t))
            (if (and res (scm-< i slen))
                (loop (scm-+ i 1) (scm-= (u32vector-ref svec i)
...
(define (bitvector-fold-left fn self x)
  (let ((len (bitvector-length self)))
    (let loop ((i 0)
               (res x))
      (if (scm-< i len)
          (loop (scm-+ i 1) (fn res (%bitvector-i-at self i)))
          res))))
...
(define (subbitarray self offset count)
  (bitvector-blit! self offset (make-bitvector (scm-- count offset)) 0 count))
